...
class XmlImporter:
    ...
    def is_enumeration(self, datatype):
        ...
        Returns:
            ...
            otherwise False.
        """
        if idaapi.get_enum(datatype) == idaapi.BADNODE:
            return False
        return True
    ...
    def is_structure(self, datatype):
        ...
        Returns:
            ...
            otherwise False.
        """
        if idaapi.get_struc_id(datatype) == idaapi.BADNODE:
            return False
        return True
    ...
    def import_compiler(self, compiler):
        ...
        if name == "Visual C++":
            ...
            comp = idaapi.COMP_WATCOM
        elif name == "GNU C++":
            comp = idaapi.COMP_GNU
        elif name == "Visual Age C++":
            comp = idaapi.COMP_VISAGE
        elif name == "Delphi":
            comp = idaapi.COMP_BP
    ...
    def import_memory_section(self, memory_section):
        ...
        s.perm = 0
        if 'r' in perms:
            s.perm |= idaapi.SEGPERM_READ
        if 'w' in perms:
            s.perm |= idaapi.SEGPERM_WRITE
        if 'x' in perms:
    ...
    def import_union(self, union):
        ...
        if idaapi.get_struc_size(sptr) < size:
            idaapi.add_struc_member(
                sptr, "", size-1, idaapi.byteflag(), None, 1)
