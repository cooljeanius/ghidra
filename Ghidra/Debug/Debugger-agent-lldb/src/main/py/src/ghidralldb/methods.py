...
PROC_WATCHES_PATTERN = extre(PROCESS_PATTERN, r"\.Watchpoints")
...
def find_proc_by_procbreak_obj(object):
    return find_proc_by_pattern(
        object, PROC_BREAKS_PATTERN, "a BreakpointLocationContainer"
    )


def find_proc_by_procwatch_obj(object):
    return find_proc_by_pattern(object, PROC_WATCHES_PATTERN, "a WatchpointContainer")
...
def detach(process: sch.Schema("Process")):
    ...
    exec_convert_errors("process detach")
...
def break_read_range(process: sch.Schema("Process"), range: AddressRange):
    ...
    sz = range.length()
    exec_convert_errors(f"watchpoint set expression -s {sz} -w read -- {offset_start}")
...
def break_write_range(process: sch.Schema("Process"), range: AddressRange):
    ...
    sz = range.length()
    exec_convert_errors(f"watchpoint set expression -s {sz} -- {offset_start}")
...
def break_write_expression(expression: str, size=None):
    """Set a watchpoint."""
    size_part = "" if size is None else f"-s {size}"
    exec_convert_errors(f"watchpoint set expression {size_part} -- {expression}")
