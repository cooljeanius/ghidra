...
def compute_ghidra_compiler(lang):
    ...
    matched_lang = sorted(
        (l for l in compiler_map if l in lang), key=lambda l: compiler_map[l]
    )
    ...
    matched_osabi = sorted(
        (l for l in comp_map if l in osabi), key=lambda l: comp_map[l]
    )
...
register_mappers = {"x86:LE:64:default": Intel_x86_64_RegisterMapper()}


def compute_register_mapper(lang):
    if lang not in register_mappers:
        if ":BE:" in lang:
