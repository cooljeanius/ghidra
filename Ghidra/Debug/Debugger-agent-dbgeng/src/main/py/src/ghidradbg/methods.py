...
PROCESSES_PATTERN = extre(SESSION_PATTERN, r"\.Processes")
PROCESS_PATTERN = extre(PROCESSES_PATTERN, "\\[(?P<procnum>\\d*)\\]")
PROC_BREAKS_PATTERN = extre(PROCESS_PATTERN, r"\.Debug.Breakpoints")
...
STACK_PATTERN = extre(THREAD_PATTERN, r"\.Stack.Frames")
...
def launch_loader(
    file: ParamDesc(str, display="File"), args: ParamDesc(str, display="Arguments") = ""
):
    """
...
def interrupt(process: sch.Schema("Process")):
    ...
    util.dbg._protected_base._control.SetInterrupt(DbgEng.DEBUG_INTERRUPT_ACTIVE)
