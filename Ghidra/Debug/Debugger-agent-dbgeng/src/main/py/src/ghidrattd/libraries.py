...
try:
    ...
    print(rf"LoadLibrary failed: {dbgdir}\dbghelp.dll {exc}")
    pass
try:
    ctypes.windll.LoadLibrary(os.path.join(dbgdir, "dbgeng.dll"))
except Exception as exc:
    print(rf"LoadLibrary failed: {dbgdir}\dbgeng.dll {exc}")
    pass
try:
    ctypes.windll.LoadLibrary(os.path.join(dbgdir, "DbgModel.dll"))
except Exception as exc:
    print(rf"LoadLibrary failed: {dbgdir}\dbgmodel.dll {exc}")
    pass
try:
    ctypes.windll.LoadLibrary(os.path.join(dbgdir, "ttd/TTDReplay.dll"))
except Exception as exc:
    print(rf"LoadLibrary failed: {dbgdir}\ttd\TTDReplay.dll {exc}")
    pass
try:
    ctypes.windll.LoadLibrary(os.path.join(dbgdir, "ttd/TTDReplayCPU.dll"))
except Exception as exc:
    print(rf"LoadLibrary failed: {dbgdir}\ttd\TTDReplayCPU.dll {exc}")
    pass
