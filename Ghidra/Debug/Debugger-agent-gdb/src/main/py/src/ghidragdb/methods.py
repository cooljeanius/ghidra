...
INF_BREAKS_PATTERN = extre(INFERIOR_PATTERN, r"\.Breakpoints")
ENV_PATTERN = extre(INFERIOR_PATTERN, r"\.Environment")
THREADS_PATTERN = extre(INFERIOR_PATTERN, r"\.Threads")
...
MEMORY_PATTERN = extre(INFERIOR_PATTERN, r"\.Memory")
MODULES_PATTERN = extre(INFERIOR_PATTERN, r"\.Modules")
MODULE_PATTERN = extre(MODULES_PATTERN, r"\[(?P<modname>.*)\]")
...
def find_inf_by_infbreak_obj(object):
    return find_inf_by_pattern(
        object, INF_BREAKS_PATTERN, "a BreakpointLocationContainer"
    )
...
def find_bptlocnum_by_obj(object):
    return find_bptlocnum_by_pattern(BREAK_LOC_PATTERN, object, "a BreakpointLocation")
...
@REGISTRY.method(
    action="launch", display="Launch at Loader", condition=util.GDB_VERSION.major >= 9
)
def launch_loader(
    inferior: sch.Schema("Inferior"),
    file: ParamDesc(str, display="File"),
...
def break_read_range(inferior: sch.Schema("Inferior"), range: AddressRange):
    ...
    gdb.execute(f"rwatch -location *((char(*)[{range.length()}]) 0x{offset_start:x})")
...
def break_write_range(inferior: sch.Schema("Inferior"), range: AddressRange):
    ...
    gdb.execute(f"watch -location *((char(*)[{range.length()}]) 0x{offset_start:x})")
...
def break_access_range(inferior: sch.Schema("Inferior"), range: AddressRange):
    ...
    gdb.execute(f"awatch -location *((char(*)[{range.length()}]) 0x{offset_start:x})")
