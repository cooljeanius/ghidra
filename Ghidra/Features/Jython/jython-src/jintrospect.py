...
def getCallTipJava(command="", locals=None):
    ...
    if not calltip[1] and not calltip[2]:
        ...
        if inspect.isclass(object):
            ...
            try:
                ...
                for constructor in constructors:
                    ...
                    tip = "{}({})".format(constructor.name, paramString)
                    tipList.append(tip)
        ...
        if len(tipList) == 0:
            if hasattr(object, "__name__") and hasattr(__builtin__, object.__name__):
                # try to get arguments for any other "old-style" builtin
                # functions (see __builtin__.java, classDictInit() method)
                methods = java.lang.Class.getMethods(org.python.core.__builtin__)
                ...
                # objects
                for (
                    possible_call_method
                ) in ghidra.python.PythonCodeCompletionFactory.getCallMethods(object):
                    signature = str(possible_call_method)
                    # clean up the method signature a bit, so it looks sane
                    signature = signature.replace("$1exposed_", ".").replace(
                        ".__call__", ""
                    )
