...
def upgrade(pip_args: List[str], dist_dir: Path, current_pyghidra_version: str) -> bool:
    from packaging.version import (
        Version,
    )  # if pyghidra imported, we know we have packaging
...
def main() -> None:
    ...
    parser.add_argument(
        "install_dir", metavar="<install dir>", help="Ghidra installation directory"
    )
    parser.add_argument(
        "-c", "--console", action="store_true", help="Force console launch"
    )
    parser.add_argument(
        "-d", "--dev", action="store_true", help="Ghidra development mode"
    )
    parser.add_argument(
        "-H", "--headless", action="store_true", help="Ghidra headless mode"
    )
