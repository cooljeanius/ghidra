...
def getYaraTargetOnDisk():
    ...
    if not os.path.exists(yaraTargetPath):
        yaraTargetPath = askFile(
            getScriptName() + ": The binary associated with the"
            " current program cannot be found.  Select the executable "
            "file that YARA will analyze.",
...
def getYaraTargetFromGhidra():
    yaraTargetPath = askFile(
        "Choose a file where Ghidra Program bytes will be saved.", "Choose file:"
    )
    if yaraTargetPath is None:
        sys.exit(1)
...
def launchYaraProcess(yaraRulePath, yaraTargetPath):
    ...
    if yaraExecutablePath is None:
        yaraExecutablePath = askFile(
            getScriptName() + ": Select the YARA executable file",
            "Select YARA executable",
        ).getPath()
        if yaraExecutablePath is None:
            sys.exit(1)
    try:
        yaraProcess = Popen(
            [yaraExecutablePath, yaraRulePath, "-sw", yaraTargetPath],
            stdout=PIPE,
            stderr=PIPE,
            bufsize=-1,
        )
    ...
    if yaraProcess.returncode != 0:
        println(
            "The YARA process failed with return code of %d.  Is there a mistake in your rule file?"
            % yaraProcess.returncode
        )
...
def setGhidraComment(memoryAddress, fileOffset, yaraRuleName):
    ...
    # the comments that already exist are separated by \n
        ...
        # the comment already contains the YARA rule name so do nothing
            ...
            println(
                "INFO: This YARA rule is already reported for this CodeUnit. "
...
def main():
    ...
    choice = askChoice(
        "Select the file that YARA will scan.",
        "Please choose one",
        choiceList,
        choiceList[0],
    )
        ...
        for key in yaraDictionary:
            for fileOffset in yaraDictionary[key]:
                ...
                # file offset matches to multiple addresses.  Let the user decide which address they want.
                    ...
                    println(
                        "WARN: The file offset "
                        + hex(myFileOffset)
                        + " matches to the following memory addresses:"
                    )
                    addressChoiceList = []
                    for addr in addressList:
                        println(
                            "\t " + mem.getBlock(addr).getName() + ":" + addr.toString()
                        )
                        addressChoiceList.append(
                            mem.getBlock(addr).getName() + ":" + addr.toString()
                        )
                    addressChoice = askChoice(
                        "Select the memory address that corresponds to the file offset: "
                        + hex(myFileOffset),
                        "Please choose one",
                        addressChoiceList,
                        addressChoiceList[0],
