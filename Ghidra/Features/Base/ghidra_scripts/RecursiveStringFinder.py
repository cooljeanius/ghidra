...
class FunctionNode(ReferenceNode):
    ...
    def indentedString(self, depth=0):
        ...
        for r in self.references:
            if r.hasString():
                string += "%s@%s - %s" % ("   " * (depth+1),
                                          r.fromAddr, r.indentedString(depth=depth+1))
        return string
        ...
        for o in opRefs:
            if o.getReferenceType().isData():
                ...
                if string is not None:
                    found.append(StringNode(insn.getMinAddress(),
    ...
    for i in range(numOperands):
        ...
        for o in opRefs:
            if o.getReferenceType().isCall():
                lst.append(FunctionNode(
                    insn.getMinAddress(), o.getToAddress()))
    return lst
