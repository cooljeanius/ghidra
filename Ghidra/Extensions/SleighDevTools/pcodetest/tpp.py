...
class tpp:
    ...
    def test_if(self, line):
        if self.data["name"]:
            ...
            sys.stderr.write(
                "ERROR: nested ifdef not allowed in file %s at line %d\n"
                % (self.fname, self.line_num)
            )
            sys.exit(1)
        else:
            self.data["ifdef"] = line.strip()
...
"""
    ...
    def test_close_brace(self):
        ...
        self.c_write("")
        if self.data["ifdef"]:
            self.c_write(self.data["ifdef"])
        self.data["num"] = str(
            len(re.findall(r"^\s+ASSERT", self.data["body"], flags=re.MULTILINE))
        )
        self.c_write(
            """#define %(name)s_NUMB %(num)s
...
%(body)s\t}
    ...
    def test_fi(self):
        ...
        if self.data["main"]:
            self.c_write(
                "\t{ %(main)s_NAME, (testFuncPtr) &%(main)s, %(main)s_NUMB },"
                % self.data
            )
        ...
        for e, f in self.info:
            if f:
                self.c_write(f)
            self.c_write("\t{{ {}_NAME, (testFuncPtr) &{}, {}_NUMB }},".format(e, e, e))
            if f:
                self.c_write("#endif /* %s */" % f)
            ...
            with open(tname) as tfile:
                for line in tfile:
                    if self.match(r"MAIN\s+(\w*).*", line):
                        extern_lines.append(
                            "\textern void %s(TestInfo* not_used);" % self.m.group(1)
                        )
...
parser = argparse.ArgumentParser(
    description="Precompile test file",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)

parser.add_argument(
    "test_file", nargs="*", help="Test file to preprocess, must end with .test"
)
parser.add_argument(
    "--entry",
    default="",
    help="Create file ENTRY contianing a main function that calls all MAIN functions",
)
