...
class PCodeTest(BuildUtil):
    ...
    def __init__(self, conf):
        ...
        (
            ...
            self.config.install_target,
        ) = self.config.toolchain.split("/")
    ...
    def print_all(cls):
        for t, pcodetest in sorted(cls.list.items(), key=lambda x: x[0].lower()):
            print(str(pcodetest))
            if pcodetest.config.verbose:
                print(pcodetest.config.dump())
...
class PCodeTestBuild(BuildUtil):
    ...
    def factory(cls, pcode_test):
        if pcode_test.config.toolchain_type == "gcc":
            ...
            raise Exception(
                pcode_test.config.format("Toolchain type %(toolchain_type)s not known")
            )
    ...
    def compile(self, input_files, opt_cflag, output_base):
        self.log_err(
            self.config.format("compile not implemented for %(toolchain_type)s")
        )
    ...
    def main(self):
        ...
        if self.config.proc_test:
            available_files.extend(
                sorted(
                    glob.glob(self.config.format("%(pcodetest_src)s/%(proc_test)s/*"))
                )
            )
        ...
        if len(skip_files) > 0:
            ...
            if len(toskip) != len(skip_files):
                self.log_warn(
                    ...
                    % " ".join([x for x in skip_files if x not in toskip])
                )
        ...
        if not self.config.has_float:
            available_files = [
                x for x in available_files if not fnmatch.fnmatch(x, "*FLOAT*")
            ]
        if not self.config.has_double:
            available_files = [
                x for x in available_files if not fnmatch.fnmatch(x, "*DOUBLE*")
            ]
        if not self.config.has_longlong:
            available_files = [
                x for x in available_files if not fnmatch.fnmatch(x, "*LONGLONG*")
            ]
        ...
        for opt_name, opt_cflag in sorted(self.config.variants.items()):
            ...
            out_name = "{}_{}_{}_pcodetest".format(
                self.config.name,
                self.config.toolchain_type.upper(),
                opt_name,
            )
            ...
            for f_test in glob.glob("*.test"):
                ...
                if self.isfile(f_h) and self.getmtime(f_test) <= self.getmtime(f_h):
                    continue
            ...
            if self.config.small_build:
                ...
                build_dir = "{}/build-PCodeTest-{}/{}".format(
                ...
                for body_file in smallFiles:
                    ...
                    if not self.isfile(companion_file) or not self.isfile(body_file):
                        self.log_info(
                            "Skipping {} {} build".format(companion_file, body_file)
                        )
                        continue
                    input_files = [
                        "pcode_test.c",
                ...
                self.log_info(
                    "Exporting {} directory to {}".format(build_dir, target_dir)
                )
...
class PCodeBuildSDCC(PCodeTestBuild):
    ...
    def cflags(self, output_file):
        ...
        f += [
            "-DHAS_LONGLONG=1"
            if self.config.has_longlong
            else "-DHAS_LONGLONG_OVERRIDE=1"
        ]
        if self.config.has_shortfloat:
            f += ["-DHAS_SHORTFLOAT=1"]
        if self.config.has_vector:
            f += ["-DHAS_VECTOR=1"]
        if self.config.has_decimal128:
            f += ["-DHAS_DECIMAL128=1"]
        if self.config.has_decimal32:
            f += ["-DHAS_DECIMAL32=1"]
        if self.config.has_decimal64:
            f += ["-DHAS_DECIMAL64=1"]
...
class PCodeBuildCCS(PCodeTestBuild):
    ...
    def compile(self, input_files, opt_cflag, output_base):
        ...
        cmd += [
            self.config.format(
                "%(toolchain_dir)s/tools/compiler/ti-cgt-msp430_16.9.0.LTS/lib/libc.a"
            )
        ]
        cmd += ["-o", output_file]
        out, err = self.run(cmd)
...
class PCodeBuildGCC(PCodeTestBuild):
    ...
    def set_library_path(self, add):
        if add and self.saved_ld_library_path:
            self.environment(
                "LD_LIBRARY_PATH", "{}:{}".format(self.config.ld_library_path, add)
            )
        elif add:
    ...
    def associated_info(self, bin, base):
        ...
        out, err = self.run([self.which("nm_exe"), "-a", bin], stdout=("%s.nm" % base))
        if err:
            self.log_warn(err)

        out, err = self.run(
            [self.which("readelf_exe"), "-a", bin], stdout=("%s.readelf" % base)
        )
        if err:
            self.log_warn(err)
    ...
    def compile(self, input_files, opt_cflag, output_base):
        ...
        cmd = (
            ...
            + [opt_cflag, "-B", self.dirname(cmp), "-o", output_file]
        )
        out, err = self.run(cmd)
        if out:
            self.log_info(out)
        ...
        if self.config.build_exe:
            ...
            if self.config.qemu_command:
                build_dir = self.build_dir(
                    self.config.build_root, "pcodetest", output_base
                )
                self.log_info(
                    self.config.format(
                        "%s %s/%s.exe"
                        % (self.config.qemu_command, build_dir, output_base)
                    )
                )
