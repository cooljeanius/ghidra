...
class BuildUtil:
    ...
    def export_file(
        ...
        try:
            ...
            self.log_err("Error occurred exporting {} to {}".format(fname, dname))
            self.log_err("Unexpected error: %s" % str(e))

    def rmtree(self, directory, verbose=True):
        if verbose:
            self.log_info("rm -r %s" % directory)
        shutil.rmtree(directory)

    def makedirs(self, directory, verbose=True):
        if verbose:
            self.log_info("mkdir -p %s" % directory)
        try:
    ...
    def copy(self, fname, dname, verbose=True):
        if not os.path.isdir(fname):
            if verbose:
                self.log_info("cp -av {} {}".format(fname, dname))
            shutil.copy(fname, dname)
        else:
            if verbose:
                self.log_info("cp -avr {} {}".format(fname, dname))
    ...
    def chdir(self, directory, verbose=True):
        if verbose:
            self.log_info("cd %s" % directory)
        os.chdir(directory)

    def remove(self, fname, verbose=True):
        if verbose:
            self.log_info("rm -f %s" % fname)
        try:
            os.remove(fname)
        except:
            pass
    ...
    def environment(self, var, val, verbose=True):
        if verbose:
            self.log_info("{}={}".format(var, val))
        os.environ[var] = val

    def unlink(self, targ, verbose=True):
        if verbose:
            self.log_info("unlink %s" % targ)
        os.unlink(targ)

    def symlink(self, src, targ, verbose=True):
        if verbose:
            self.log_info("ln -s {} {}".format(src, targ))
    ...
    def build_dir(self, root, kind, what):
        return (
            root
            + "/"
            + re.sub(r"[^a-zA-Z0-9_-]+", "_", "build-{}-{}".format(kind, what))
        )
    ...
    def open_log(self, root, kind, what, chdir=False):
        ...
        self.log_info("{} LOGFILE {}".format(self.log_prefix(kind, what), logFile))

        try:
            self.rmtree(build_dir, verbose=False)
        except:
            pass
        self.makedirs(build_dir, verbose=False)
        self.log_open(logFile)
        if chdir:
            self.chdir(build_dir)
    ...
    def mkinfo(self, fname):
        ...
        for s in typedefs:
            if s in ifdefs:
                f.write("#ifdef %s\n" % ifdefs[s])
            f.write(
                '_Static_assert(sizeof(%s) == %d, "INFO %s should have size %d, is not correct\\n");\n'
